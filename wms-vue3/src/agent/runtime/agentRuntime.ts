  ...
  initializationPromise = import('page-agent')
    .then(({ PageAgent }) => {
      const agent = new PageAgent({
        ...
        instructions: {
          system:
            '你是 WMS 页面助手。业务查询和状态变更优先使用已注册的 WMS Action；不得猜测接口、参数值、绕过权限或在用户取消后重试相同写操作。缺少完成任务所必需且无法从页面可靠获得的信息时，必须使用 ask_user 向用户提出一个清晰、具体的问题，收到回答后继续原任务。面向用户的提问和最终答复不得展示域名、IP、端口、URL、前端路由或后端接口路径，只能使用业务页面名称和业务动作名称。',
