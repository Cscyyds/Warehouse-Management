 ...
 *   - 模块:menuPermissionMap.PAGE_MENU_BY_TITLE(页面标题 → 后端模块菜单名)
...
import { PAGE_MENU_BY_TITLE } from './menuPermissionMap'
...
/** 判定是否为权限叶子:标准树带 type 标记,legacy 兜底树按 perm_ 前缀识别 */
function isPermNode(node: PermissionTreeNode): boolean {
  return node.type === 'perm' || String(node.id || '').startsWith('perm_')
}
...
export function groupRolePermissionTree(source: PermissionTreeNode[]): GroupedPermNode[] {
  ...
  // 3) 组装模块节点;未被页面分组收编的权限挂「其他权限」(保留原层级)
  const result: GroupedPermNode[] = []
  for (const moduleName of moduleOrder) {
    result.push({ id: `module:${moduleName}`, label: moduleName, children: modules.get(moduleName)! })
  }
