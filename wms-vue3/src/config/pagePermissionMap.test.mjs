...
const SHARED_VIEW_ALLOWED = [
  ...
  // 覆盖 13 类单据 + 概览 + 未绑品号清单共 15 个页面，共享 view 是该模块的既定设计（非复制粘贴错误）
  [
    '生产概览', '未绑品号清单', '成品缴库单', '生产领料单', '生产退料单', '生产补料单',
    '非生产领料单', '非生产退料单', '托工领料单', '托工退料单', '托工补料单', '托外加工缴回单',
    '物料切割单', '托工退回单', '销售退回单',
  ],
  // 贸易数据：后端只有 perm_trade_view / perm_trade_manage 两个聚合码，
  // 覆盖 4 类单据 + 单据详情共 5 个页面，共享 view 是该模块的既定设计
  ['采购订单（天心）', '采购退货单（天心）', '销售订单（天心）', '销售退货单（天心）', '贸易单据详情'],
