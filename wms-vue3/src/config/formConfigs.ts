...
export type FieldType = 'input' | 'textarea' | 'select' | 'radio' | 'tree-select' | 'date' | 'number' | 'section' | 'input-suffix' | 'dynamic-table' | 'embedded-table'
    ...
    tabs: [
      {
        ...
        fields: [
          ...
          { key: 'orgId', label: '归属机构', type: 'input-suffix', required: true, placeholder: '请选择归属机构', suffixIcon: 'Tree', span: 12, treeData: [{ id: 'root', name: '百诺全屋五金配套服务商', children: [{ id: '0', name: '总经办', children: [{ id: '1', name: '销售部' }, { id: '2', name: '仓储物流部' }, { id: '3', name: '客服部' }, { id: '4', name: '产品部', children: [{ id: '4-1', name: '采购部' }, { id: '4-2', name: '售后部' }] }, { id: '5', name: '行政部' }, { id: '6', name: '财务部' }] }] }], loadTreeData: async () => { const res = await getOrgTree(); return res.data.tree } },
          { key: 'companyId', label: '归属公司', type: 'input-suffix', placeholder: '请选择归属公司', suffixIcon: 'Tree', span: 12, treeData: [{ id: 'root', name: '百诺全屋五金配套服务商', children: [{ id: '0', name: '总经办', children: [{ id: '1', name: '销售部' }, { id: '2', name: '仓储物流部' }, { id: '3', name: '客服部' }, { id: '4', name: '产品部', children: [{ id: '4-1', name: '采购部' }, { id: '4-2', name: '售后部' }] }, { id: '5', name: '行政部' }, { id: '6', name: '财务部' }] }] }], loadTreeData: async () => { const res = await getOrgTree(); return res.data.tree } },
          ...
          { key: 'roles', label: '角色分配', type: 'embedded-table', columns: [{ key: 'roleName', label: '角色名称' }, { key: 'checked', label: '是否分配', type: 'checkbox' }], tableData: [
