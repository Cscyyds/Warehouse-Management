  ...
  <div class="detail-page">
    ...
    <div class="page-body">
      <ListTemplate
        ...
        :export-file-name="`供应商赠送金额明细_${supplierName || supplierId}`"
      ...
      >
        <template #search>
          <el-form :model="searchForm" inline size="default" label-width="80px">
            ...
            <el-form-item label="业务类型"><el-input v-model="searchForm.bizType" placeholder="如 SUPPLIER_GIFT_MANUAL" clearable style="width:200px" /></el-form-item>
...
import { getSupplierGiftLogList, searchSupplierGiftLogs, type SupplierGiftLogItem } from '@/api'
...
const tableData = ref<SupplierGiftLogItem[]>([])
...
const fallbackData: SupplierGiftLogItem[] = [
  { log_id: 'sbl_demo_1', supplier_id: '', biz_no: 'SL202606300001', bill_no: 'SL202606300001', biz_type: 'SUPPLIER_GIFT_MANUAL', account_item: 'GIFT', record_type: 'ADD', amount: '500.00', before_amount: '0.00', after_amount: '500.00', created_by_name: '示例', created_at: '', remark: '兜底数据' },
]
...
async function loadData() {
  ...
  try {
    if (searchForm.recordType || searchForm.bizType || hasRange) {
      ...
      const res = await searchSupplierGiftLogs({
      ...
      const res = await getSupplierGiftLogList({
        supplier_id: supplierId.value,
        page: pagination.page,
        page_size: pagination.pageSize,
        sort_by: sortBy.value || undefined,
