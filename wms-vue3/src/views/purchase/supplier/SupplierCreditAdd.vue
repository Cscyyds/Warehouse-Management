...
import { addSupplierCreditLog } from '@/api'
...
async function handleSubmit() {
  ...
  try {
    const res = await addSupplierCreditLog({
      supplier_id: formData.supplierId,
      amount: formData.amount,
      remark: formData.remark || undefined,
    })
    ElMessage.success(`保存成功，单据号：${res.data.bill_no}，当前授信余额：${res.data.new_balance}`)
    router.push('/purchase/supplier/credit')
  } catch {
