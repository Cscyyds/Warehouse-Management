  ...
  <ListTemplate title="产品销售汇总表" v-model:page="pagination.page" v-model:page-size="pagination.pageSize" :total="pagination.total" @page-change="loadData">
    ...
    <template #table>
      <el-table :data="tableData" stripe size="small" style="width:100%" row-class-name="table-row" show-summary :summary-method="getSummaries">
        ...
        <el-table-column prop="netQuantity" label="净销售数量" width="80" align="center" />
        <el-table-column prop="netAmount" label="净销售金额" width="80" align="center" />
      </el-table>
    </template>
  </ListTemplate>
</template>
...
async function loadData() {
  try { const res = await getSalesProductReport({ ...searchForm, page: pagination.page, pageSize: pagination.pageSize } as SalesQueryParams); tableData.value = (res.data as any).list || []; pagination.total = (res.data as any).total || 0 }
