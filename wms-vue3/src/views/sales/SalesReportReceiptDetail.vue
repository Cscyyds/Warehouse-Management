  ...
  <ListTemplate title="订单收款明细表" v-model:page="pagination.page" v-model:page-size="pagination.pageSize" :total="pagination.total" @page-change="loadData">
    ...
    <template #table>
      <el-table :data="tableData" stripe size="small" style="width:100%" row-class-name="table-row" show-summary :summary-method="getSummaries">
        ...
        <el-table-column prop="receiptNo" label="收款单号" min-width="130" />
        <el-table-column prop="receiptDate" label="收款日期" width="110" />
        <el-table-column prop="receiptAmount" label="收款金额" width="80" align="center" />
        <el-table-column prop="paymentMethod" label="收款方式" width="80" />
        <el-table-column prop="bankName" label="结算银行" width="80" />
        <el-table-column prop="totalAmount" label="订单总额" width="80" align="center" />
        <el-table-column prop="paidAmount" label="已收金额" width="80" align="center" />
        <el-table-column prop="unpaidAmount" label="未收金额" width="80" align="center" />
        <el-table-column prop="receiptStatus" label="收款状态" width="80" align="center">
...
async function loadData() {
  try { const res = await getSalesReconciliationReport({ ...searchForm, page: pagination.page, pageSize: pagination.pageSize } as SalesQueryParams); tableData.value = (res.data as any).list || []; pagination.total = (res.data as any).total || 0 }
