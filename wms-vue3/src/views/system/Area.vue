<template>
  ...
  >
    ...
    <template #table>
      <el-table :data="tableData" stripe size="small" style="width:100%" row-class-name="table-row" row-key="area_id" border>
        <el-table-column type="index" label="" width="55" align="center" />
        <el-table-column prop="area_name" label="区划名称" min-width="140" />
        <el-table-column prop="area_code" label="区划编码" width="130" />
        <el-table-column prop="area_type_label" label="区划类型" width="110" align="center">
...
async function loadData() {
  try {
    ...
    if (hasSearch) {
      ...
      const res = await getAreaList({ page: pagination.page })
      tableData.value = flattenTree(res.data.area || [])
      pagination.total = res.data.total || 0
    }
  } catch {
