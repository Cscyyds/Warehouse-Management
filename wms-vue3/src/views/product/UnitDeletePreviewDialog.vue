      ...
      <div class="preview-section">
        ...
        <div class="preview-card">
          <div class="preview-target">
            <span class="label">删除单位：</span>
            <el-tag type="danger" size="small">{{ unit?.unit_name || '-' }}</el-tag>
          </div>
          ...
          <div v-if="productCount > 0 && !loading" class="cascade-hint">
            该单位作为主计量单位被 {{ productCount }} 个产品引用。
          </div>
        </div>
      </div>
...
const props = defineProps<{
  modelValue: boolean
  /** 待删除的单位（至少含 unit_id / unit_name） */
  unit?: ProductUnitItem | null
}>()
...
async function onOpen() {
  resetState()
  if (!props.unit?.unit_id) {
    ElMessage.warning('未指定删除的计量单位')
    handleClose()
    return
  }
  loading.value = true
  const unitId = props.unit.unit_id
  await Promise.allSettled([
    loadProductCount(unitId),
...
/** 统计使用该单位作为主计量单位的产品数量，并缓存产品 ID 列表 */
async function loadProductCount(unitId: string) {
  try {
    const res = await searchProduct({
      search_field: JSON.stringify(['unit_id']),
      search_value: JSON.stringify({ unit_id: unitId }),
      page: 1,
      page_size: 999
    })
...
/** 加载单位选项列表（排除当前待删单位） */
async function loadUnitOptions(excludeId: string) {
  try {
    const res = await getProductUnitList({ page: 1, page_size: 999 })
    const all = res.data.unit || []
    unitOptions.value = all
      .filter((u: ProductUnitItem) => u.unit_id !== excludeId)
      .map((u: ProductUnitItem) => ({ label: u.unit_name, value: u.unit_id }))
  } catch {
    unitOptions.value = []
  }
}
...
function validate(): boolean {
  if (migrateChecked.value && !targetUnitId.value) {
    ElMessage.warning('请选择目标单位')
    return false
  }
  if (migrateChecked.value && targetUnitId.value === props.unit?.unit_id) {
    ElMessage.warning('迁移目标不能与当前删除的单位相同')
    return false
  }
...
async function handleConfirm() {
  ...
  try {
    // 若启用迁移，先执行（接口32）
    if (migrateChecked.value && targetUnitId.value) {
      await migrateProductUnit({
        source_unit_id: unitId,
        change_message: JSON.stringify([{ new_unit: targetUnitId.value, change_products: productIds.value }])
      })
    }
    // 删除单位
