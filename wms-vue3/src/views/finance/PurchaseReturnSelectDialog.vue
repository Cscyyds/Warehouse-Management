    ...
    <div class="select-layout">
      ...
      <div class="right-panel">
        ...
        <ul class="selected-list">
          <li v-for="(item, idx) in selected" :key="item.purchase_return_id" class="selected-item">
...
const emit = defineEmits<{
  ...
  'confirm': [item: PurchaseReturnListItem]
  'confirmMultiple': [items: PurchaseReturnListItem[]]
}>()
...
function handleReset() { filter.return_no = ''; filter.supplier_name = ''; handleSearch() }

function handleSelectionChange(val: PurchaseReturnListItem[]) {
  selected.value = val
}

function handleRowClick(row: PurchaseReturnListItem) {
  if (props.multiple) {
...
function handleConfirm() {
  if (selected.value.length === 0) {
    ElMessage.warning('请至少选择一个采购退货单')
    return
  }
  if (props.multiple) {
