<template>
  <div class="customer-prepayment-usage">
    <el-card class="customer-summary">
      <el-descriptions title="客户预付款概况" :column="4" border>
        <el-descriptions-item label="客户名称">{{ customerSummary?.customer_name ?? '-' }}</el-descriptions-item>
        <el-descriptions-item label="预付款余额">{{ formatAmount(customerSummary?.prepayment_amount) }}</el-descriptions-item>
        <el-descriptions-item label="累计已使用">{{ formatAmount(customerSummary?.cumulative_used_prepayment_amount) }}</el-descriptions-item>
        <el-descriptions-item label="累计已新增">{{ formatAmount(customerSummary?.cumulative_added_prepayment_amount) }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
    ...
    <el-card class="usage-table" style="margin-top: 16px">
      ...
      >
        <el-table-column prop="bill_no" label="单据编号" width="180" show-overflow-tooltip />
        ...
        <el-table-column prop="created_by_name" label="操作人" width="120" show-overflow-tooltip />
...
import { getPrepaymentUsageList, type PrepaymentUsageItem } from '@/api'
...
const tableData = ref<PrepaymentUsageItem[]>([])
...
async function loadData() {
  loading.value = true
  try {
    const res = await getPrepaymentUsageList({
