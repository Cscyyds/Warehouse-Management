<template>
  <div class="customer-credit-usage">
    <el-card class="customer-summary">
      <el-descriptions title="客户授信概况" :column="4" border>
        <el-descriptions-item label="客户名称">{{ customerSummary?.customer_name ?? '-' }}</el-descriptions-item>
        <el-descriptions-item label="授信额度上限">{{ formatAmount(customerSummary?.credit_limit) }}</el-descriptions-item>
        <el-descriptions-item label="已用额度">{{ formatAmount(customerSummary?.used_credit_amount) }}</el-descriptions-item>
        <el-descriptions-item label="剩余额度">{{ formatAmount(customerSummary?.remaining_credit_amount) }}</el-descriptions-item>
      </el-descriptions>
    </el-card>
...
import { getCreditUsageList, type CreditUsageItem } from '@/api'
...
const tableData = ref<CreditUsageItem[]>([])
...
async function loadData() {
  loading.value = true
  try {
    const res = await getCreditUsageList({
