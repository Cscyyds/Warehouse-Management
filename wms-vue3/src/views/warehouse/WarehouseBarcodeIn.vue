  ...
  >
    ...
    <template #table>
      <el-table border :data="tableData" stripe size="small" style="width:100%" row-class-name="table-row" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="color" label="颜色" show-overflow-tooltip width="70" />
        <el-table-column prop="unit" label="计量单位" show-overflow-tooltip width="80" />
        <el-table-column prop="origin" label="原产地" show-overflow-tooltip min-width="80" />
        <el-table-column prop="quantity" label="数量" show-overflow-tooltip width="70" align="center" />
...
async function loadData() {
  loading.value = true
  try {
    const res = await getInboundBarcodeList({ ...searchForm, type: '入库', page: pagination.page, page_size: pagination.pageSize, pageSize: pagination.pageSize })
