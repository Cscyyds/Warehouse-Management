...
function handleTenantDropdownVisible(visible: boolean) {
  if (visible && !loadingTenants.value) loadTenants()
}
...
async function submitTenant() {
  ...
  try {
    ...
    if (!tenantOptions.value.some((item) => item.id === tenantResult.value?.tenant_code)) {
      tenantOptions.value.unshift({ id: tenantResult.value.tenant_code, name: tenantResult.value.tenant_name })
    }
      ...
      <div class="operation-card">
        ...
        <el-form ref="subscriptionFormRef" :model="subscriptionForm" :rules="subscriptionRules" label-position="top" class="dense-form">
          <el-form-item label="租客编码" prop="tenant_id">
            <el-select v-model="subscriptionForm.tenant_id" filterable :loading="loadingTenants" placeholder="选择可用租客" @visible-change="handleTenantDropdownVisible">
